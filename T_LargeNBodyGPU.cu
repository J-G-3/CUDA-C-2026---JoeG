...
__global__ void calculateForce(float3 *, float3 *, float *, float , float , int );
__global__ void updatePositionAndVelocity(float3 *, float3 *, float3 *, float* , float, float ,float , int);
...
// divide leapFrog among 2 kernals
__global__ void calculateForce(float3 *p, float3 *f, float *m, float g, float h, int n)
{
    // Global thread index
...
__global__ void updatePositionAndVelocity(float3 *p, float3 *v, float3 *f, float* m, float damp, float dt,float t, int n)
{
 int i = blockIdx.x*blockDim.x + threadIdx.x;
...
void nBody()
{
	...
	while(t < RUN_TIME) //CALL TWO FUNCS HERE
	{
		// leapFrog<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, Damp, dt, t, N);

        // calc force
        
        calculateForce<<<GridSize,BlockSize>>>(PGPU,FGPU, MGPU, G, H, N);
        cudaDeviceSynchronize(); // wait for device to stop
        cudaErrorCheck(__FILE__, __LINE__);

        // Calc new pos and vel
        updatePositionAndVelocity<<<GridSize,BlockSize>>>( PGPU, VGPU, FGPU, MGPU, Damp, dt, t, N);
        cudaDeviceSynchronize(); // wait for device to stop
        cudaErrorCheck(__FILE__,__LINE__);
