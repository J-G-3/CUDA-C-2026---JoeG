...
void setUpDevices()
{
	BlockSize.x = 1024; //changing block size to 1024 (max threads for modern GPUs x direction); 1500 invalid
	BlockSize.y = 1;	//max 1024
	BlockSize.z = 1;	//max 64
	
	GridSize.x = 1; //just need 1 block for our problem
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{	
	...
	// Device "GPU" Memory
	cudaMalloc(&A_GPU,N*sizeof(float)); //used cudaMalloc to allocate memory on the GPU and we use & instead of * for floats
	cudaMalloc(&B_GPU,N*sizeof(float)); //notice we do not set = but include array in cudaMalloc statement cudaMalloc(&A_GPU...)
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n) //changing this kernel to cover all threads
{
...
//Kernal Time
//  the kernal runs once per thread, all of these threads run in parallel, 1024 copies of the kernal all run at once
//  thread 0 with  tid = 0, thread n tid = n; 
//  stride = 1024, i = i +1024, this updates the vector element for the remaining 476 elements
//  local variables are remembered for each iteration but are forgotton after the instance, only the global will remain
//  therefore, when i = 2048 stop updating 
//  key takeaway: each thread has its own kernal instance, the local variables persist for the lifetime of that kernal instance
...
int main()
{
	...
	cudaMemcpyAsync(C_CPU, C_GPU, N*sizeof(float), cudaMemcpyDeviceToHost); //you copy data from 2nd argument to 1st
																			//example cudaMemcpyAsync(destination, data, size in bytes, GPU(device) to CPU(host))
	// Making sure the GPU and CPU wiat until each other are at the same place.
	cudaDeviceSynchronize(); //device and host data sync
