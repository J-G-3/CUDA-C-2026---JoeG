...
// Defines
#define NUMBER_OF_RANDOM_NUMBERS 2147483 // number of random numbers
#define NUMBER_OF_BINS 10 // number of ranges
...
{
	cudaDeviceProp prop;
	cudaGetDeviceProperties(&prop, 0); // get properties of GPU and insert into props location to directly edit it 
	cudaErrorCheck(__FILE__, __LINE__);
	
	BlockSize.x = 222;
	if(prop.maxThreadsDim[0] < BlockSize.x) // threads in the x direction < defined block size then error message 
	{
	...
	if(prop.maxGridSize[0] < GridSize.x) // defined grid size is bigger than set one on the device 
	{
...
void fillHistogramCPU()
{
	float breakPoint;
	int k, done;
	float stepSize = MAX_RANDOM_NUMBER/(float)NUMBER_OF_BINS; // dividing the ranges amongst the bins 
	
	for(int i = 0; i < NUMBER_OF_RANDOM_NUMBERS; i++) // for the number of random numbers, 
	{
		breakPoint = stepSize; // when to move on to the next bin 
		k = 0; // bin idx
		done =0;
		while(done == 0)
		{
			...
			k++; // increase the bin idx each time the number is greater than the current breakpoint
			breakPoint += stepSize; // next breakpoint
		}
	}
}
...
{
    __shared__ int localHistogram[NUMBER_OF_BINS]; // each block contains its own temp histogram

    int tid = threadIdx.x + blockIdx.x * blockDim.x; // global index

    if (threadIdx.x < NUMBER_OF_BINS) 
    {
        localHistogram[threadIdx.x] = 0; // zeroing the local histogram vector
    }
    ...
    {
        int bin = (int)(randomNumbers[tid] / (MAX_RANDOM_NUMBER / NUMBER_OF_BINS)); // determines which number goes into which bin
                                                                                    // if the random number is 37.5/10 = 3.75; goes into bin 3 range(30-40)

        if (bin >= NUMBER_OF_BINS)
        {
            bin = NUMBER_OF_BINS - 1; // if bin is greater than 10 (Number_of_Bins); no out of bounds error
        }

        // 
        atomicAdd(&localHistogram[bin], 1); // adding one to the bin, we got a value within that range 
    }
    ...
    if (threadIdx.x < NUMBER_OF_BINS)
    {
        atomicAdd(&hist[threadIdx.x], localHistogram[threadIdx.x]); // adding bin occurences to the global histogram vector 
                                                                    // threadIdx.x is the bin idx and adding like bins together 
    }
}
...
int main()
{
    ...
    // call the kernal to fill the histogram on the GPU
