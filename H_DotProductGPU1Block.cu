...
{	
	// Host "CPU" memory.				
	A_CPU = (float*)malloc(N*sizeof(float)); // allocating a place for each vector in the CPU
	B_CPU = (float*)malloc(N*sizeof(float));
	C_CPU = (float*)malloc(N*sizeof(float));
	
	// Device "GPU" Memory
	cudaMalloc(&A_GPU,N*sizeof(float)); // allocating memory for the memory in VRAM
	cudaErrorCheck(__FILE__, __LINE__); // refresher: returns error; then we assign A_GPU with & the first element of start of mem address
...
void innitialize()
{
	for(int i = 0; i < N; i++)
	{		
		A_CPU[i] = (float)i;	 // regular vector of floats no multiple
		B_CPU[i] = (float)(2*i); // creating a vector and multiplying each element by two( vector of even elements)
	}
}
...
void dotProductCPU(float *a, float *b, float *C_CPU, int n)
{
	...
	{ 
		C_CPU[id] = a[id] * b[id]; // just mul
	}
	
	for(int id = 1; id < n; id++)
	{ 
		C_CPU[0] += C_CPU[id]; // dumping every number into the zeroth element(adding zeroth element to every element)
	}
}
...
{
	int tid = threadIdx.x; // only 1D
 

    // each thread is only computing one product
    if (tid < n)
    {
        C_GPU[tid] = a[tid] * b[tid]; // calculate all the products using Input arrays [a0*b0 a1*b1 a2*b2...]

    }

    // syncing threads (making sure that all multiplications have finished first)
    __syncthreads();
 ...
 { 
    ...
    if(odd == 1 && tid == 0)
    {
        C_GPU[0] += C_GPU[active -1]; // last element added to first element 
    }
