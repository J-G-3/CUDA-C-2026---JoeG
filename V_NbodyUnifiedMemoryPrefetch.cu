...
void drawPicture()
{
    ...
    cudaDeviceSynchronize(); // ensure GPU finished before drawing
...
void setup()
{
	...
	GridSize.z = 1;
        ...
        // unified mem; All devices use the same mem no need for MPVF per GPU
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		...
		for(int i = 0; i < NumberOfGpus; i++)
            ...
            // Prefetch to this GPU before compute
            cudaMemPrefetchAsync(P, N*sizeof(float3), i);
            cudaErrorCheck(__FILE__, __LINE__);
            cudaMemPrefetchAsync(V, N*sizeof(float3), i);
            cudaErrorCheck(__FILE__, __LINE__);
