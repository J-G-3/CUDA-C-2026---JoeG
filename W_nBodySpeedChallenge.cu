...
void timer();
void setup();
__global__ void getForces(float3 *, float3 *, float, float, int);
...
// USING SHARED MEM IN GETFORCES


__global__ void getForces(float3 *p, float3 *f, float g, float h, int n)
{
	 __shared__ float3 ps[BLOCK_SIZE];

    int i = threadIdx.x + blockDim.x * blockIdx.x;

    float3 pi = p[i]; // copying pos of bodies to pi
    ...
    {
        ps[threadIdx.x] = p[tile + threadIdx.x];
        __syncthreads();




         #pragma unroll 8 // one loop does  bodies of work at once
        for(int j = 0; j < BLOCK_SIZE; j++)
        {
           ...
           // if(globalj != i)
            //{
                float dx = ps[j].x - pi.x;
                float dy = ps[j].y - pi.y;
                float dz = ps[j].z - pi.z;
            
                float d2 = dx*dx + dy*dy + dz*dz+ 1e-10f;
               // float d = sqrtf(d2);
                float invD = rsqrtf(d2); // reciprical of sqrt(d2) = 1/d
                float invD2 = invD * invD; // 1/d2
                float invD3 = invD2 * invD; // 1/d3
                float invD5 = invD3 * invD2; // 1/d5

                float force =  g * invD3 - h * invD5; // got srid of extra divisions in fx fy and fz


               // float force_mag = G/(d2) - H/(d2*d2);

                fx += force * dx; // we account for the d up there 
                fy += force * dy;
                fz += force * dz;
        //    }
        }
...
{	
	...
	if(i < n)
	{
		if(t == 0.0f)
		{
			v[i].x += ((f[i].x-damp*v[i].x))*dt/2.0f; // got rid of m[i] bc it equals 1
			v[i].y += ((f[i].y-damp*v[i].y))*dt/2.0f;
			v[i].z += ((f[i].z-damp*v[i].z))*dt/2.0f;
		}
		else
		{
			v[i].x += ((f[i].x-damp*v[i].x))*dt; // mass initialized as one 
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		getForces<<<GridSize,BlockSize>>>(PGPU,FGPU, G, H, N); // v not even used 
		cudaErrorCheck(__FILE__, __LINE__);
...
{
    // CHECKING FOR BOTH POWER OF 2 AND THE BOTHS
    ...
    {
	    printf("\n ERROR: N must be a power of 2 and satisfy 256 < N < 262144.\n");
	    exit(0);
    }
