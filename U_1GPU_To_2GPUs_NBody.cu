...
float3 *P, *V, *F;
float *M; 
// Declaring the variables for each GPU
...
void setup()
{
    	...
    	// setting the Half N to be the size of the N0 and other half N1 (subtracting to account for odd N's);  
    	N = 1000;
    	HalfN = N/2;
        N0 = HalfN;
        N1 = N - HalfN;
	...
	GridSize.x = (N0 - 1)/BlockSize.x + 1; //why is this based off of N0
	GridSize.y = 1;
	GridSize.z = 1;
    ...
    cudaSetDevice(0);
    cudaMalloc(&MGPU0,N*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&PGPU0,N*sizeof(float3));
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&VGPU0,N*sizeof(float3));
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&FGPU0,N*sizeof(float3));
	cudaErrorCheck(__FILE__, __LINE__);
...
__global__ void getForces(float3 *p, float3 *v, float3 *f, float *m, float g, float h, int start, int count, int n)
{
	...
	int i = threadIdx.x + blockDim.x*blockIdx.x + start; // using start input for offset, 

	if(i < start + count && i < n) // if i is less than its responsible range (start+count) and less that n amount
	{
		f[i].x = 0.0f;
		f[i].y = 0.0f;
		f[i].z = 0.0f;
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
        // general scheme calculate forces
        // calculate body
        // draw pic (copies in the drawpicture() function)

        // get forces and bodies first half GPU0
        cudaSetDevice(0);
		getForces<<<GridSize,BlockSize>>>(PGPU0, VGPU0, FGPU0, MGPU0, G, H, 0, N0, N);
		cudaErrorCheck(__FILE__, __LINE__);
		moveBodies<<<GridSize,BlockSize>>>(PGPU0, VGPU0, FGPU0, MGPU0, Damp, dt, t, 0, N0, N);
		cudaErrorCheck(__FILE__, __LINE__);

        // get forces and bodies second half GPU1
        cudaSetDevice(1);
		getForces<<<GridSize,BlockSize>>>(PGPU0, VGPU0, FGPU0, MGPU0, G, H, HalfN, N1, N);
		cudaErrorCheck(__FILE__, __LINE__);
		moveBodies<<<GridSize,BlockSize>>>(PGPU0, VGPU0, FGPU0, MGPU0, Damp, dt, t, HalfN, N1, N);
		cudaErrorCheck(__FILE__, __LINE__);
        ...
        // transferring GPU mem 
        cudaSetDevice(0);
		cudaMemcpyAsync(&PGPU0[HalfN], &PGPU1[HalfN], N1*sizeof(float3), cudaMemcpyDeviceToDevice); // copying 2nd half into first GPU
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(&VGPU0[HalfN], &VGPU1[HalfN], N1*sizeof(float3), cudaMemcpyDeviceToDevice); // 
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(&FGPU0[HalfN], &FGPU1[HalfN], N1*sizeof(float3), cudaMemcpyDeviceToDevice); // 
		cudaErrorCheck(__FILE__, __LINE__);

		cudaSetDevice(1);
		cudaMemcpyAsync(&PGPU1[0], &PGPU0[0], N0*sizeof(float3), cudaMemcpyDeviceToDevice); // 1st half copied to the 2nd GPU
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(&VGPU1[0], &VGPU0[0], N0*sizeof(float3), cudaMemcpyDeviceToDevice);
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(&FGPU1[0], &FGPU0[0], N0*sizeof(float3), cudaMemcpyDeviceToDevice);
		cudaErrorCheck(__FILE__, __LINE__);

        // Syncing the devices so that they copy mem and are ready 
        cudaSetDevice(0);
		cudaDeviceSynchronize();
		cudaErrorCheck(__FILE__, __LINE__);
